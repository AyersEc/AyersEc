<div align="center">

# Sandro Jose
### Senior Security Engineer · DFIR Leader · TAM Lead

*Building resilient security programs. Turning adversary intelligence into decisive action.*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-sandro--jose-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sandro-jose/)
[![Location](https://img.shields.io/badge/Canada-🇨🇦-FF0000?style=flat-square)]()
[![CISSP](https://img.shields.io/badge/CISSP-2025-005A8E?style=flat-square&logo=isc2&logoColor=white)]()

</div>

---

## About me

Senior Security Engineer and DFIR Leader with a career built across incident response, enterprise security governance, and critical infrastructure protection. Currently leading the TAM team and DFIR function at **ARANCIA**, serving as senior escalation point for high-severity incidents across healthcare, financial, and industrial sectors.

Strong hands-on background with the **Microsoft Security ecosystem** — from large-scale Defender deployments protecting 5,000+ assets to KQL-based threat hunting and SOC detection engineering. I translate technical risk into executive decisions and build security programs that scale.

---

## Core expertise

| DFIR & Threat Intelligence | Microsoft Security Ecosystem |
|---|---|
| Digital Forensics & IR | Microsoft Sentinel (SIEM/SOAR) |
| Threat Hunting (KQL) | Defender for Endpoint/Identity |
| Detection Engineering | Defender for Cloud Apps/Office |
| MITRE ATT&CK · NIST | Microsoft Entra ID · Intune |
| Cyber Threat Intelligence | Purview · CrowdStrike Falcon |
| Velociraptor · Eric Zimmerman Tools | EDR · SIEM & Log Analytics |

| Security Governance | Infrastructure & OT |
|---|---|
| Zero Trust Architecture | OT/ICS Security (Purdue Model) |
| Security Strategy & Policy | Nozomi · EDR |
| IAM · PAM · DLP | Critical Infrastructure |
| Compliance & Risk Management | Industrial Plant Security |
| SOC Operations | M&A Security Assessments |

---

## Certifications

| Certification | Issuer | Year |
|---|---|---|
| CISSP | ISC2 | 2025 |
| CompTIA Security+ | CompTIA | 2024 |
| Cyber Intelligence Professional | ADINT | 2024 |
| Digital Forensics Expert | Digital Forensic Academy | 2023 |
| Digital Forensics | SEC4US | 2023 |

---

## Featured project

### [AURA — Adversary Unified Risk Assessment](https://github.com/AyersEc/AURA)

> CLI-based threat intelligence platform that scores every known threat actor against your organization's profile using MITRE ATT&CK, Ransomware.live, and CISA KEV data.

---

## Languages

🇧🇷 Portuguese — Fluent &nbsp;|&nbsp; 🇺🇸 English — Fluent

---

<div align="center">
<sub>Security is not a product. It's a process.</sub>
</div>
